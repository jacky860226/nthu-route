...
:inputFileName(inputFileName),
 ...
 delims("(,)- \n"),
 fh_(resultFileName, FileHandler::AutoFileType),
 rr(NULL),
 netWires(NULL),
 netTileNum(NULL),
 routingSpace_(NULL),
 ...
 viaMaxOverflow(0),
 wireBendCount(0),
 netHasUnconnectedPin(0),
 netHasUnconnectedWire(0),
 netHasDuplicateWireCount(0)
{}
...
void Verifier::initialVerifierMemorySpace () {
    ...
    routingSpace_ = new vector< Plane<RoutingTile, RoutingEdge> > (
        this->rr->get_layerNumber(), Plane<RoutingTile, RoutingEdge>(
                this->rr->get_gridx(),
                this->rr->get_gridy(),
                RoutingTile(-1, 0),
                RoutingEdge(-1, 0),
                3   //Because we need use this plane to check duplicate wires, so
...
void Verifier::parseOneNetWires(int netPos){
    ...
    if(strcmp(netName.c_str(), rr->get_netName(netPos)) != 0) {
        cout << "The " << netId << "nd net of testcase and result file are not the same."
             << endl;
    ...
    */
	...
	while(keepRead == true){
		...
		if(strncmp(token,"!", 1) == 0) keepRead = false;
		else{
			//x of first cell
			x1 = atoi(token);
			//y of first cell
...
bool Verifier::isSegmentOverflow(int x1, int y1, int z1, int x2, int y2, int z2){
	...
	if(x1 != x2 && y1 == y2 && z1 == z2){
		...
		for(int i = 0; i < segmentLength; ++i){
			if(this->rr->capacity(z1, x1 + i, y1, x1 + i + 1, y1) <
                (*routingSpace_)[z1].edge(x1 + i, y1, DIR_EAST).usage
			){
				return true;
			}
		}
	...
	//y-direction wire segment
		...
		//z1--;
		for(int i = 0; i < segmentLength; ++i){
			if(this->rr->capacity(z1, x1, y1 + i, x1, y1 + i + 1) <
                (*routingSpace_)[z1].edge(x1, y1 + i, DIR_NORTH).usage
			){
				return true;
			}
			return false;
		...
		for(int x = 0; x < this->rr->get_gridx() - 1; ++x){
			for(int y = 0; y < this->rr->get_gridy(); ++y){
				...
				if(curCapacity > maxCapacity){
					...
					this->overflow += diff;
					//update max overflow
					if(diff > this->maxOverflow) maxOverflow = diff;
				}
			}
		}
        ...
        for(int x = 0; x < this->rr->get_gridx(); ++x){
            ...
            for(int y = 0; y < this->rr->get_gridy() ; ++y){
                ...
                if(curCapacity > maxCapacity){
                    ...
                    viaOverflow += diff;
                    //update max overflow
                    if(diff > viaMaxOverflow) viaMaxOverflow = diff;
                }
			}
		}
	}
...
bool Verifier::routeWireOnMaps(int netPos, int x, int y, int z, 
                               int length, int dir)
{
    int xBasicShift = 0;
    int yBasicShift = 0;
    ...
    for(int i = 0; i < length; ++i){
        if((*routingSpace_)[z + zTotalShift].edge(
                x + xTotalShift,
                y + yTotalShift,
                dirType).color != netPos)
        {
            (*routingSpace_)[z + zTotalShift].edge(
                    x + xTotalShift,
                    y + yTotalShift,
                    dirType).color = netPos;
            ...
            if(static_cast<int>(dirType) < 4) {
                ...
                (*routingSpace_)[z + zTotalShift].edge(
                        x + xTotalShift, 
                        y + yTotalShift,
                        dirType).usage += rr->get_layerMinWidth(z + zTotalShift);

                //Add min. wire spacing to usage
                (*routingSpace_)[z + zTotalShift].edge(
                        x + xTotalShift, 
                        y + yTotalShift,
                        dirType).usage += rr->get_layerMinSpacing(z + zTotalShift);
            }else{
                ++viaWireLength;
            }
        }else{
        ...
        if((*routingSpace_)[z + zTotalShift].vertex(
                    x + xTotalShift,
                    y + yTotalShift).color != netPos)
        {
            (*netTileNum)[netPos]++;
           (*routingSpace_)[z + zTotalShift].vertex(
                    x + xTotalShift,
                    y + yTotalShift).color = netPos;

            ++(*routingSpace_)[z + zTotalShift].vertex(
                    x + xTotalShift,
                    y + yTotalShift).viaCutCount;
        }
        
        if((*routingSpace_)[z + zTotalShift + zBasicShift].vertex(
                    x + xTotalShift + xBasicShift,
                    y + yTotalShift + yBasicShift).color != netPos)
        {
            (*netTileNum)[netPos]++;
            (*routingSpace_)[z + zTotalShift + zBasicShift].vertex(
                    x + xTotalShift + xBasicShift,
                    y + yTotalShift + yBasicShift).color = netPos;

            ++(*routingSpace_)[z + zTotalShift + zBasicShift].vertex(
                    x + xTotalShift + xBasicShift,
                    y + yTotalShift + yBasicShift).viaCutCount;
        }
...
bool Verifier::isNeighborTileSameNet(int curX, int curY, int curZ, 
                                     int netPos, int dirId){
	int colorID = -1;
	switch(dirId){
		case 0:
			if(curY >= this->rr->get_gridy() - 1) return false;
...
WireSegment::WireSegment(int x1, int y1, int z1, 
...
int WireSegments::lineDirection(int x1, int y1, int z1, 
                                int x2, int y2, int z2)
{
	//x direction
	if(x1 != x2 && y1 == y2 && z1 == z2){
...
bool WireSegments::add_segment(int x1, int y1, int z1, int x2, int y2, int z2){
	...
	if(wireDirection != 0){
		switch(wireDirection){
			case 1:
				this->segments.push_back(WireSegment(
						min(x1, x2), y1, z1, 
						max(x1, x2), y1, z1, wireDirection)
				);
				return true;
				break;
			case 2:
